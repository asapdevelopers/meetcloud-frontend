...
export default class SettingsPopup extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    if (nextProps.settings) {
      this.setState({
        selectedAudioInput: nextProps.settings.audioDeviceSelected.deviceId
      });
      this.setState({
        selectedAudioOutput: nextProps.settings.audioDeviceSinkSelected.deviceId
      });
      this.setState({
        selectedVideoInput: nextProps.settings.videoDeviceSelected.deviceId
      });
    }
  }
  ...
  saveSettings = () => {
    if (this.props.settings.audioDevicesList) {
      const selectedAudioInput = this.props.settings.audioDevicesList.find(
        x => x.deviceId === this.state.selectedAudioInput
      );
      this.props.onAudioInputSelected(selectedAudioInput);
    }

    if (this.props.settings.videoDevicesList) {
      const selectedVideoInput = this.props.settings.videoDevicesList.find(
        x => x.deviceId === this.state.selectedVideoInput
      );
      this.props.onVideoInputSelected(selectedVideoInput);
    }

    if (this.props.settings.audioDevicesSinkList) {
      const selectedAudioOutput = this.props.settings.audioDevicesSinkList.find(
        x => x.deviceId === this.state.selectedAudioOutput
      );
      this.props.onAudioOutputSelected(selectedAudioOutput);
    }
